...
Two cards ship with the integration and register themselves — no separate HACS
install, and no way for the cards and the integration to drift to different
versions.
...
**`custom:emhass-deferrable-card`** — one deferrable load: whether it should run
now, its scheduled power, next start, runtime today, a timeline of its
