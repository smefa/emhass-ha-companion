...
| **0.18.2** | Same as 0.18.1, plus the peak is priced on the tariff's own measurement interval (`capacity_charge_interval_timesteps` + `capacity_charge_current_interval_history`) rather than the optimizer timestep, closing the 0.18.1 over-shaving gap |
| **0.18.3+** | Multi-component demand charges: `capacity_cost_per_kw` may be a list of independent rates (peak + shoulder, winter peak + all-year peak). Mean-of-top-3 stays one component at `rate / n`, never K=3. Below 0.18.3 a K>1 profile falls back to the hard-cap union of every component window |
